...
public class AvailableTimesController {
	...
	void initialize() {
		...
		avaibleTimesButton.setOnAction(new EventHandler<ActionEvent>() {
			...
			@Override
			public void handle(ActionEvent event) {
				App.switchScene(new AvailableTimesController(userModel.getUser()), "AvailableTimes.fxml");
			}
		});
	...
	private void initializeComboBoxesAndCheckBox() {
	...
	private String AvailableTimesToString() {
		...
		for (AvailableTime availableTime : userModel.getAvailableTimes()) {
